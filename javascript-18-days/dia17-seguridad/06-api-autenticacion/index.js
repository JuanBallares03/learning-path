...
app.post('/login', (pedido, respuesta) => {
    mySQL.connection.query(
        'SELECT id FROM usuarios WHERE user = "' + pedido.body.user + '" AND pass = "' + pedido.body.pass + '"',
...
app.post("/validate", seguridad.validarToken, (pedido, respuesta) => {
    mySQL.connection.query(
        'SELECT p.page FROM permisos p JOIN permisosxusuario u ON u.permiso_id = p.id ' +
        'WHERE u.usuario_id = "' + pedido.user.usuario_id + '" AND p.name = "' + pedido.body.permiso + '"',
        function (error, resultados) {
            if (error) throw error;
            if (resultados.length === 0) {
                respuesta.send(undefined);
